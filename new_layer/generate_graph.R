...
# Main function to generate graph with hidden nodes
generateGraphWithHiddenNodes <- function(k) {
  h <- floor(k / 3) # Set h as the floor of k divided by 3
  
  # Calculate h within the pruferwithskeleton function, so it's not needed here as a parameter
  result <- pruferwithskeleton(k,h)
  while(length(result$ConditionOne) < 2 || length(result$ConditionTwo) < 2) {
    result <- pruferwithskeleton(k,h)
  }
  
  result$Hidden <- unique(result$Hidden)
  ...
  # Return both the graph and the numbers (identifiers) of hidden nodes
  return(list(Graph = g_directed, HiddenNodes = hidden_nodes))
